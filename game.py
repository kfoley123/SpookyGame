...
eventOne = {
    "choice": "you come to a fork in the road choose left or right: ",
    "correctChoice": "right",
    "incorrectChoice": "left",
    "correctResult": "eventTwo",
    "incorrectResult": "death",
}

eventTwo = { 
    "choice": "You see a chest on the side of the path. Do you open it? ",
    "correctChoice": "no",
    "incorrectChoice": "yes",
    "correctResult": "endGame",
    "incorrectResult": "death",
}
...
while(lives > 0 and currentEvent != endGame):

    currentChoice = input(currentEvent["choice"])
    currentChoice = formatAnswerString(currentChoice)
    if(currentChoice == currentEvent["correctChoice"]):
        currentEvent = eventMap[currentEvent["correctResult"]]
    elif(currentChoice == currentEvent["incorrectChoice"]):
        if(currentEvent["incorrectResult"] == "death"):
            lives = deathScreen(lives)
        else: 
            currentEvent = eventMap[currentEvent["incorrectResult"]]
